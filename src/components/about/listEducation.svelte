<script lang="ts">
	...
	import { BiMapPin, BiCollapse, BiLinkExternal } from 'svelte-icons-pack/bi';
	import Map from '../map.svelte';

	export let educationIntitution: string;
	export let yearsOfEducation: string[];
...
<div class="mt-4 flex flex-col items-start">
	<div class="flex items-center">
		...
		<img src={logoUrl} alt="" class="h-[50px] w-[50px] object-cover rounded-full shadow-lg mx-[-12px]" />
	</div>

	<div class="flex justify-between items-center mt-2 relative">
		<span class="font-bold text-md text-gray-500">{yearsOfEducation.join(' - ')} </span>
	...
	<p class="text-left text-gray-600 mb-4 mt-2 text-lg">
		{description}
	</p>
</div>
